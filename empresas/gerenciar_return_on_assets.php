...
try {
    $stmt = $pdo->prepare(
        'SELECT id_ticker, end_date, return_on_assets
         FROM financial_data_history_quarterly
    ...
    $historico = array_map(
        static function (array $registro): array {
            return [
                ...
                'return_on_assets' => $registro['return_on_assets'] !== null ? (float) $registro['return_on_assets'] : null,
            ];
        },
        $registros
    );
    ...
    responderJson(500, ['erro' => 'Não foi possível carregar o histórico de return_on_assets.', 'detalhes' => $e->getMessage()]);
}
